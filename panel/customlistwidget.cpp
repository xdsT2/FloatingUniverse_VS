...
void CustomListWidget::wheelEvent(QWheelEvent *event)
{
    if (enabledSmoothScroll)
    {
        ...
        else if (event->angleDelta().y() < 0) // 下滚
        {
...
{
    pressGlobalPos = e->globalPosition().toPoint();
    emit pressedEvent();
...
void CustomListWidget::mouseReleaseEvent(QMouseEvent *e)
{
    if (e->button() == Qt::LeftButton
            && (e->globalPosition().toPoint() - pressGlobalPos).manhattanLength() < QApplication::startDragDistance())
    {
        emit releasedEvent();
    }
