...
ImageItem::ImageItem(QWidget *parent) : ResizeableItemBase(parent)
{
    ...
    layout->setContentsMargins(selectBorder, selectBorder, selectBorder, selectBorder);
...
void ImageItem::setImage(const QString& imageName)
{
    ...
    {
        QString imagePath = rt->PANEL_FILE_PATH + imageName;
        QPixmap originalPixmap(imagePath);
        if (!originalPixmap.isNull()) {
            // 生成缩略图：限制最大尺寸，使用平滑缩放算法保证质量
            QSize thumbSize(256, 256);
            QPixmap thumbPixmap = originalPixmap.scaled(thumbSize, Qt::KeepAspectRatio, Qt::SmoothTransformation);
            imageLabel->setPixmap(thumbPixmap);
            // 不再保存原始大图，节省内存
