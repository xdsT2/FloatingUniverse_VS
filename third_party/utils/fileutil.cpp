...
QString readTextFile(QString path, QString codec)
{
    ...
    if (!codec.isEmpty() && codec != "UTF-8")
    {
        ...
        text_stream.setCodec(QTextCodec::codecForName(codec.toLatin1()));
#endif
    }
...
{
    return writeTextFile(path, text, "UTF-8");
}
...
QString readTextFileWithFolder(QString file, QString folder, QString codec)
{
...
bool recycleFile(const QString &filename)
{
    ...
    // pFrom 需要双 null 结尾
    QString fromPath = filename + QString(QChar('\0'));
    wchar_t* fromBuf = new wchar_t[fromPath.length() + 1];
    fromPath.toWCharArray(fromBuf);
    fromBuf[fromPath.length()] = L'\0';

    memset(&opRecycle, 0, sizeof(opRecycle));
    opRecycle.hwnd              = nullptr;
    opRecycle.wFunc             = FO_DELETE;
    opRecycle.pFrom             = fromBuf;
    opRecycle.pTo               = nullptr;
    opRecycle.fFlags            = FOF_ALLOWUNDO | FOF_NOCONFIRMATION | FOF_SILENT;
    opRecycle.hNameMappings     = nullptr;
    opRecycle.lpszProgressTitle = nullptr;
