...
FacileMenu::FacileMenu(QWidget *parent) : QWidget(parent)
{
    ...
    // 获取窗口尺寸
    QWidget* parentWidget = this->parentWidget() ? this->parentWidget() : this;
    QScreen* parentScreen = parentWidget->screen();
    int screen_number = QGuiApplication::screens().indexOf(parentScreen);
    if (screen_number < 0)
        screen_number = 0;
...
void FacileMenu::getBackgroupPixmap()
{
    ...
    // 屏幕截图
    QPixmap bg = screen->grabWindow(0, rect.left(), rect.top(), rect.width(), rect.height());
        ...
        if (!all_menu_same_color || !parent_menu) // 如果是统一判断，那么由最外层计算
        {
            ...
            if (ok)
            {
                ...
                if (auto_theme_by_bg)
                {
                    ...
                    // 设置颜色
                    m_bg_color = img_bg;
                    // 确保前景色与背景色有足够对比度（明度差至少80）
                    double bgLuminance = ImageUtil::calculateLuminance(img_bg);
                    double fgLuminance = ImageUtil::calculateLuminance(img_fg);
                    if (qAbs(fgLuminance - bgLuminance) < 80)
                    {
                        // 对比度不足，根据背景明暗选择白/黑
                        img_fg = (bgLuminance < 128) ? QColor(255, 255, 255) : QColor(0, 0, 0);
                    }
